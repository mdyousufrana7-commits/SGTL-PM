...
/* ---------------- permission helpers ---------------- */
function requireAuth(req, queryToken) {
  const token = auth.getBearerToken(req) || queryToken || null;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const user = requireAuth(req, u.searchParams.get('token'));
    ...
    // Downloadable CSV report: material summary + full usage log for a project
    const reportMatch = p.match(/^\/api\/projects\/(\d+)\/report\.csv$/);
    if (reportMatch && method === 'GET') {
      const projectId = Number(reportMatch[1]);
      const project = db.getProject(projectId);
      if (!project) return json(res, 404, { error: 'Project not found' });
      if (user.role === 'engineer' && project.engineer_username !== user.username) {
        return json(res, 403, { error: 'Not assigned to this project' });
      }
      const materials = db.listMaterials(projectId).map(m => {
        const used = db.totalUsed(m.id);
        return { ...m, total_used: used, remaining_quantity: (m.planned_quantity || 0) - used };
      });
      const byId = Object.fromEntries(materials.map(m => [m.id, m]));
      const usageRows = db.usageForProject(projectId, 5000).map(r => ({ ...r, material_name: byId[r.material_id]?.name, unit: byId[r.material_id]?.unit }));
      const csv = reports.materialUsageCsv(project, materials, usageRows);
      const safeName = project.name.replace(/[^a-z0-9]+/gi, '-').toLowerCase();
      return send(res, 200, csv, {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="material-report-${safeName}.csv"`
      });
    }
